...
#define CALIBRATION_VALUE 48 // Calibration value for converting ADC result to voltage
...
unsigned int result, volt; // Variables for ADC result and voltage calculation
...
void main(void) {
    ...
    while (1) { // Infinite loop
        ...
        result = ((unsigned int) adc_high << 8) | ((unsigned int) adc_low); // Combine high and low bytes to form the full ADC result

        volt = ((result * CALIBRATION_VALUE) / NUMBER_OF_BIT); // Convert ADC result to voltage
...
void lcd_init(void) {
    // Configure I/O pins
    TRISA = 0x01; // Set RA0 as input (for ADC), others as output
...
void lcd_output(unsigned int i) {
    ...
    // Display each digit with leading spaces
    if (d4 > 0) {
